...
def process_checkpoint(
    ...
    try:
        ...
        print(
            f"Warning: Missing dependency {e}. Attempting to load with weights_only=True"
        )
    ...
    if remove_cls_token and "eva.pos_embed" in modified_state_dict:
        ...
        modified_state_dict["eva.pos_embed"] = modified_state_dict["eva.pos_embed"][
            :, 1:, :
        ]
        new_shape = modified_state_dict["eva.pos_embed"].shape
        print(
            f"Removed CLS token from positional embeddings: {original_shape} -> {new_shape}"
        )
    elif not remove_cls_token and "eva.pos_embed" in modified_state_dict:
        print(
            f"Keeping CLS token in positional embeddings: {modified_state_dict['eva.pos_embed'].shape}"
        )
    else:
