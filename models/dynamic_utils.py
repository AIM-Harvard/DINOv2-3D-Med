...
def resample_abs_pos_embed(
    ...
    if (
        num_new_tokens == num_pos_tokens
        and new_size[0] == new_size[1]
        and new_size[1] == new_size[2]
    ):
        return posemb
    ...
    posemb = posemb.reshape(1, old_size[0], old_size[1], old_size[2], -1).permute(
        0, 4, 1, 2, 3
    )
...
def resample_patch_embed(
    ...
    def resize(x_np, _new_size):
        ...
        x_upsampled = F.interpolate(x_tf, size=_new_size, mode=interpolation)[
            0, 0, ...
        ].numpy()
        return x_upsampled
